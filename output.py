...
class Output():
    ...
    def add(self, title, url, magnets, error):
        ...
        event.event.dispatch('AddingData', title=title,
                             url=url, magnets=magnets)
        self.resource_list.append(
            {'title': title, 'url': url, 'magnets': magnets, 'error': error})
        self.need_check = True
    ...
    def save_json(self, f):
        ...
        json.dump(
            ...
            sort_keys=config.output['json_sort_keys'],
            ensure_ascii=config.output['json_ensure_ascii'])
            ...
            for resource in self.resource_list:
                ...
                for added_resource in new_resource_list:
                    if added_resource['title'] == resource['title']:
                        ...
                        added_resource['magnets'] = list(
                            set(added_resource['magnets']))
                        break
                if add_flag:
