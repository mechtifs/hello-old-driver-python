...
class Analyzer():
    ...
    def get_magnet_links(self, result_text):
        ...
        # 40-char magnets
        hashes = list(
            set(re.findall(config.analyzer['regex_magnets_40'], result_text)))
        # 32-char magnets
        hashes.extend(
            list(set(re.findall(config.analyzer['regex_magnets_32'], result_text))))
        magnets = list(set(
            [(config.analyzer['magnet_prefix'] + hash_value).lower() for hash_value in hashes]))
